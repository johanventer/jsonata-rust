...
enum Binding<'a> {
    Variable(JsonValue),
    Function(&'a dyn Fn(Vec<&JsonValue>) -> JsonValue)
}
...
impl<'a> Frame<'a> {
    ...
    pub fn new_from(parent_frame: &'a Frame<'a>) -> Frame {
        Self {
            bindings: HashMap::new(),
    ...
    pub fn bind(&mut self, name: &str, value: Binding<'a>) {
...
impl<'a> JsonAta<'a> {
    pub fn new(expr: &str) -> JsonAta<'a> {
        let mut environment = Frame::new();

        // TODO: Apply statics to the environment
        environment.bind("sum", Binding::Function(&sum));
...
mod tests {
    ...
    fn bind_and_lookup() {
        ...
        assert_eq!(frame.lookup("bool").unwrap().as_var().as_bool().unwrap(), true);
        assert_eq!(frame.lookup("number").unwrap().as_var().as_number().unwrap(), 42);
        assert_eq!(frame.lookup("string").unwrap().as_var().as_str().unwrap(), "hello");
    ...
    fn lookup_through_parent() {
        ...
        assert_eq!(child.lookup("value").unwrap().as_var().as_number().unwrap(), 42);
    }
