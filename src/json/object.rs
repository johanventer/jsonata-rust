...
/// A binary tree implementation of a string -> `Value` map. You normally don't
/// have to interact with instances of `Object`, much more likely you will be
/// using the `Value::Object` variant, which wraps around this struct.
#[derive(Debug)]
pub struct Object {
    store: Vec<Node>,
}
...
impl Object {
    ...
    pub fn override_last(&mut self, value: Value) {
    ...
    /// will return the `Value` stored behind the `key`.
    pub fn remove(&mut self, key: &str) -> Option<Value> {
        ...
        for (i, node) in self.store.iter_mut().enumerate() {
            if i == index {
                ...
                removed = Some(mem::replace(&mut node.value, Value::Null));
            } else {
                let value = mem::replace(&mut node.value, Value::Null);
